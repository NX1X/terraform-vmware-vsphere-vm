...
> provided as-is, without updates or support. Feel free to fork it.

For Virtual Machine Provisioning with (Linux/Windows) customization. This configuration deploys single or multiple VMs to vSphere using the `vsphere_virtual_machine` resource, and is verified against Terraform 1.9.7 with the `hashicorp/vsphere` provider 2.10.0.

## Background

This was built and used at **Ben-Gurion University (BGU)** to automate Windows and
Linux VM provisioning on our vSphere environment, as part of moving infrastructure
provisioning to Terraform. It is shared here as a snapshot of that work; it reflects
our setup at the time and is no longer actively maintained.
...
[Apache-2.0](LICENSE). This configuration is based on the MIT-licensed [Terraform-VMWare-Modules/terraform-vsphere-vm](https://github.com/Terraform-VMWare-Modules/terraform-vsphere-vm) module; that license notice is retained in [NOTICE](NOTICE).
